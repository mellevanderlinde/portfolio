...
Next, the IAM role can be assumed from a GitHub Actions workflow. 
This can be done with the action [`configure-aws-credentials`](https://github.com/aws-actions/configure-aws-credentials), making `.github/workflows/oidc.yml` look as follows.
...
The last step is to invoke AWS actions from the workflow. The `cdk diff` command can be run by adding the following step.
...
This results in the following `.yml` file.
